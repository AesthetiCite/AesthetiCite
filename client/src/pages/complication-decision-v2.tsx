...
const QUICK_OPTIONS = [
  ...
  { id: "necrosis",           label: "Skin Necrosis",         query: "skin necrosis impending necrosis after filler management treatment", urgent: true,  icon: AlertTriangle },
  { id: "vision_change",      label: "Vision Change",         query: "vision change ocular compromise after filler management",            urgent: true,  icon: Eye },
  ...
  return (
    <div className={`rounded-xl border border-white/8 bg-slate-900 border-l-4 ${a.border} overflow-hidden`}>
      <button
        ...
        data-testid={`section-toggle-${number}`}
      >
    ...
    <div className="space-y-3">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <p className="text-white font-bold text-lg leading-tight" data-testid="text-protocol-name">{r.matched_protocol_name}</p>
          <p className="text-white/40 text-xs mt-0.5">Confidence: {Math.round(r.confidence * 100)}%</p>
        </div>
        <div className="flex flex-col items-end gap-1.5 flex-shrink-0">
          <span
            className={`px-2.5 py-1 rounded text-white text-xs font-bold ${urgencyStyle.bg} ${urgencyStyle.pulse ? "animate-pulse" : ""}`}
    ...
    <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center px-4 py-12">
        ...
        {QUICK_OPTIONS.map((opt) => {
          ...
          return (
            <button
              ...
              data-testid={`button-complication-${opt.id}`}
      ...
      <div className="w-full max-w-xl flex gap-2">
        <input
          ...
          disabled={isLoading}
          data-testid="input-custom-complication"
        ...
        <Button
          ...
          data-testid="button-search-complication"
        >
          {isLoading
...
export default function ComplicationDecisionV2Page() {
  ...
  // Parse ?complication= from URL and auto-fire
  useEffect(() => {
    const p = new URLSearchParams(window.location.search);
    const c = p.get("complication");
    if (c && !result) {
      const opt = QUICK_OPTIONS.find((o) =>
        o.id.toLowerCase() === c.toLowerCase() ||
        o.label.toLowerCase().includes(c.toLowerCase())
      );
  ...
  if (!isLoading && !result) {
    return (
      <div className="min-h-screen bg-slate-950">
        <ComplicationSelector onQuery={handleQuery} isLoading={isLoading} />
      </div>
    );
  }
  if (isLoading) {
    return (
      <div className="min-h-screen bg-slate-950">
        <LoadingSkeleton complication={currentQuery} />
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-slate-950 text-white" ref={topRef}>
        ...
        <div className="max-w-2xl mx-auto px-4 py-3 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-1.5 flex-shrink-0">
            {result && (
              ...
              }`} data-testid="badge-sticky-urgency">
      ...
      {result && (
        <div className="max-w-2xl mx-auto px-4 py-4 space-y-3">
          ...
          {/* 1 — DIAGNOSIS */}
          <Section
            number="1" title="Diagnosis" icon={CheckCircle2} accent={urgencyAccent}
          ...
          <Section number="2" title="Immediate Actions" icon={Zap} accent={urgencyAccent}>
            <div className="-mx-4 -mb-4">
              <WorkflowRunner
                complication={result.matched_protocol_key}
          ...
          {/* 4 — SAFETY & ESCALATION — open unless routine */}
          <Section
            number="4" title="Safety & Escalation" icon={Shield} accent="orange"
            defaultOpen={result.risk_assessment.urgency !== "routine"}
          >
            {safetyData ? (
              <SafetyEscalationBlock
                safety={safetyData}
                className="bg-transparent border-0 shadow-none -mx-4 -mb-4"
              />
            ) : (
              <div className="space-y-2">
          ...
          <Section
            number="5" title="Evidence" icon={BookOpen} accent="violet" defaultOpen={false}
          ...
          {/* 6 — DOCUMENTATION — collapsed by default */}
          <Section number="6" title="Documentation" icon={FileText} accent="emerald" defaultOpen={false}>
            <div className="space-y-3">
