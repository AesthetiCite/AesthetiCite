...
import {
  ...
  FileText, Waves, GitBranch
} from "lucide-react";

function getPrimaryActions(corpusLabel: string) { return [
  {
    id: "search",
    href: "/ask",
    icon: Search,
    label: "Evidence Search",
    description: `Ask any clinical question. Get cited, evidence-graded answers from ${corpusLabel} of peer-reviewed literature.`,
    tag: "Core",
...
const SECONDARY_TOOLS = [
  ...
  { href: "/decide", icon: Microscope, label: "Complication Differential" },
  { href: "/workflow", icon: GitBranch, label: "Workflow Runner" },
  { href: "/hyaluronidase", icon: Waves, label: "Hyaluronidase Calc" },
...
const EMERGENCY_SHORTCUTS = [
  { label: "Vascular Occlusion", query: "vascular_occlusion_ha_filler",              color: "text-red-600 bg-red-50 border-red-200" },
  { label: "Anaphylaxis",        query: "anaphylaxis_allergic_reaction",              color: "text-red-600 bg-red-50 border-red-200" },
  { label: "Skin Necrosis",      query: "skin_necrosis_after_filler",                 color: "text-red-700 bg-red-50 border-red-300" },
  { label: "Vision Change",      query: "vision_change_after_filler",                 color: "text-red-700 bg-red-50 border-red-300" },
  { label: "Tyndall Effect",     query: "tyndall_effect_ha_filler",                   color: "text-sky-600 bg-sky-50 border-sky-200" },
  { label: "Ptosis",             query: "botulinum_toxin_ptosis",                     color: "text-amber-600 bg-amber-50 border-amber-200" },
  { label: "Nodules",            query: "filler_nodules_inflammatory_or_noninflammatory", color: "text-slate-600 bg-slate-50 border-slate-200" },
...
export default function ClinicHomePage() {
  ...
  const corpusLabel = corpusStats?.papers_inserted
    ? `${Math.floor(corpusStats.papers_inserted / 1000)}K+ papers`
    : "825K+ papers";
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <header className="border-b border-slate-200 bg-white sticky top-0 z-20">
        <div className="mx-auto max-w-5xl px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <Link href="/emergency">
              <button
                data-testid="button-emergency-home"
                className="flex items-center gap-1.5 bg-red-600 hover:bg-red-700 text-white text-xs font-bold rounded-lg px-3 py-1.5 transition-colors shadow-sm"
              >
      ...
      <div className="mx-auto max-w-5xl px-4 py-8 space-y-8">
        <div>
          <h1 className="text-2xl font-black text-slate-800 tracking-tight" data-testid="text-greeting">{greeting}.</h1>
          <p className="text-sm text-slate-500 mt-1">What do you need today?</p>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {getPrimaryActions(corpusLabel).map((action) => {
            const Icon = action.icon;
            return (
              <Link key={action.id} href={action.href}>
                <div
                  data-testid={`card-primary-${action.id}`}
                  className={`
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
          ...
          <div className="flex flex-wrap gap-2">
            {EMERGENCY_SHORTCUTS.map((s) => (
              <Link key={s.query} href={`/emergency?protocol=${s.query}`}>
                <button
                  data-testid={`button-shortcut-${s.query}`}
                  className={`text-xs font-semibold border rounded-lg px-3 py-2 transition-all hover:opacity-80 ${s.color}`}
                >
                  {s.label}
                </button>
              </Link>
        ...
        <p className="text-[10px] text-slate-400 text-center pb-4">
          AesthetiCite — {corpusLabel} indexed · Clinical safety &amp; evidence engine for aesthetic injectables.
