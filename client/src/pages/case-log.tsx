...
import {
  ...
  FileText, AlertTriangle, Activity, X
} from "lucide-react";
...
const PROTOCOL_OPTIONS = [
  { value: "vascular_occlusion_ha_filler", label: "Vascular Occlusion" },
  { value: "anaphylaxis_allergic_reaction", label: "Anaphylaxis / Hypersensitivity" },
  { value: "tyndall_effect_ha_filler", label: "Tyndall Effect" },
  { value: "botulinum_toxin_ptosis", label: "Ptosis / Toxin Complication" },
  { value: "infection_or_biofilm_after_filler", label: "Infection / Biofilm" },
  { value: "filler_nodules_inflammatory_or_noninflammatory", label: "Filler Nodules / Granuloma" },
...
const PROTOCOL_COLORS: Record<string, string> = {
  vascular_occlusion_ha_filler: "bg-red-100 text-red-700",
  anaphylaxis_allergic_reaction: "bg-red-100 text-red-700",
  tyndall_effect_ha_filler: "bg-sky-100 text-sky-700",
  botulinum_toxin_ptosis: "bg-amber-100 text-amber-700",
  infection_or_biofilm_after_filler: "bg-orange-100 text-orange-700",
  filler_nodules_inflammatory_or_noninflammatory: "bg-slate-100 text-slate-700",
...
export default function CaseLogPage() {
  ...
  function resetForm() {
    ...
    setSelectedSymptoms([]); setOutcome("");
    ...
    <div className="min-h-screen bg-slate-50">
      <header className="sticky top-0 z-20 border-b border-slate-200 bg-white/95 backdrop-blur-sm">
        <div className="mx-auto max-w-5xl px-4 py-3 flex items-center justify-between">
          ...
          <button
            data-testid="button-log-new-case"
      ...
      <div className="mx-auto max-w-5xl px-4 py-6 space-y-6">
            ...
            <div>
              ...
              <div className="flex gap-4 mt-3">
                <div className="text-center">
                  <p className="text-lg font-black" data-testid="text-total-cases">{stats?.total_cases ?? "—"}</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm">
              <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-3 flex items-center gap-1.5">
                <BarChart3 className="w-3 h-3" /> By Complication
              </h3>
              <div className="space-y-2.5">
                {Object.entries(stats.by_protocol).sort((a, b) => b[1] - a[1]).slice(0, 6).map(([key, count]) => (
                  <DistributionBar key={key} label={key} count={count} total={stats.total_cases} color="bg-slate-700" />
                ))}
              </div>
            </div>
            <div className="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm">
              <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-3 flex items-center gap-1.5">
                <Activity className="w-3 h-3" /> By Region
              </h3>
              <div className="space-y-2.5">
                {Object.entries(stats.by_region).sort((a, b) => b[1] - a[1]).slice(0, 6).map(([key, count]) => (
                  <DistributionBar key={key} label={key} count={count} total={stats.total_cases} color="bg-red-400" />
                ))}
              </div>
            </div>
            <div className="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm">
              <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-3 flex items-center gap-1.5">
                <FileText className="w-3 h-3" /> By Procedure
              </h3>
              <div className="space-y-2.5">
                {Object.entries(stats.by_procedure).sort((a, b) => b[1] - a[1]).slice(0, 6).map(([key, count]) => (
                  <DistributionBar key={key} label={key} count={count} total={stats.total_cases} color="bg-amber-400" />
                ))}
              </div>
            </div>
          </div>
        ...
        {!statsLoading && stats && stats.total_cases === 0 && (
          <div className="bg-white rounded-2xl border border-dashed border-slate-200 p-12 text-center">
            ...
            >
              <Plus className="w-3.5 h-3.5" /> Log First Case
            </button>
          </div>
        )}
      </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {SYMPTOM_OPTIONS.map(s => (
                    <button
                      key={s}
                      data-testid={`button-symptom-${s.toLowerCase().replace(/[^a-z0-9]/g, "-")}`}
                ...
                <div className="bg-amber-50 border border-amber-200 rounded-xl px-3 py-2.5 mb-4">
                  <div className="flex items-start gap-2">
                    ...
                    <p className="text-xs text-amber-700">
                      Do not enter patient names, dates of birth, or any identifying information. Log clinical data only.
                    </p>
                  </div>
                </div>
                <button
