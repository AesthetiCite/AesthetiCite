...
import {
  ...
  AlertTriangle, ChevronRight, Calculator,
} from "lucide-react";
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div>
          ...
          <Select value={region} onValueChange={setRegion}>
            <SelectTrigger className="mt-1 h-9 text-sm" data-testid="select-hyal-region">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
        ...
        <div>
          ...
          <Select value={severity} onValueChange={setSeverity}>
            <SelectTrigger className={`mt-1 h-9 text-sm ${isVO ? "border-red-400" : ""}`} data-testid="select-hyal-severity">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
        ...
        <div>
          ...
          <Select value={String(concentration)} onValueChange={(v) => setConcentration(Number(v))}>
            <SelectTrigger className="mt-1 h-9 text-sm" data-testid="select-hyal-conc">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              {CONC_OPTIONS.map((o) => (
        ...
        <CardContent className="p-4 space-y-3">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="bg-blue-50 dark:bg-blue-950/20 rounded-lg p-3 text-center">
              ...
              <p className={`text-2xl font-bold ${isVO ? "text-red-600" : "text-blue-600"}`} data-testid="text-hyal-dose">
            ...
            <div className="bg-muted/50 rounded-lg p-3 text-center">
              ...
              <p className="text-2xl font-bold" data-testid="text-hyal-volume">{mlNeeded}</p>
          ...
          <Button size="sm" variant="outline" onClick={copy} className="w-full gap-1.5" data-testid="button-copy-dose">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <Select value={String(vialUnits)} onValueChange={(v) => setVialUnits(Number(v))}>
            <SelectTrigger className="mt-1 h-9 text-sm" data-testid="select-vial-size"><SelectValue /></SelectTrigger>
            <SelectContent>
              {VIAL_SIZES.map((v) => (
        ...
        <div>
          ...
          <Select value={String(diluent)} onValueChange={(v) => setDiluent(Number(v))}>
            <SelectTrigger className="mt-1 h-9 text-sm" data-testid="select-diluent"><SelectValue /></SelectTrigger>
            <SelectContent>
      ...
      <Card className="border-blue-200 dark:border-blue-800">
        <CardContent className="p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <p className="text-2xl font-bold text-blue-600" data-testid="text-concentration">{unitsPerMl.toFixed(1)} U/ml</p>
          </div>
            ...
            <tbody>
              {tableRows.map((r) => (
                <tr key={r.vol} className="border-b last:border-0">
                  ...
                  <td className="py-1.5 text-right font-bold" data-testid={`text-units-${r.vol}`}>{r.units}U</td>
                </tr>
              ))}
            </tbody>
          </table>

          <Button size="sm" variant="outline" onClick={copy} className="w-full mt-3 gap-1.5" data-testid="button-copy-table">
            ...
            <CardContent className="p-3">
              <div className="flex items-start justify-between gap-3">
                ...
                <Button
                  ...
                  onClick={() => copy(row)}
                  data-testid={`button-copy-row-${row.region.replace(/\s+/g, "-").toLowerCase()}`}
                >
      ...
      <div className="flex flex-wrap gap-2">
        {Object.entries(PROTOCOLS).map(([key, p]) => (
          <button
            ...
            data-testid={`button-protocol-${key}`}
        ...
        <Card className={`border-2 ${URGENCY_STYLES[protocol.urgency]}`}>
          ...
          <CardContent className="px-4 pb-4 space-y-3">
            ...
            <div className="flex gap-2">
              <Button size="sm" variant="outline" onClick={copy} className="flex-1 gap-1.5" data-testid="button-copy-protocol">
                <Copy className="h-3.5 w-3.5" /> Copy Protocol
              </Button>
              <Link href={`/decide?complication=${encodeURIComponent(protocol.name)}`}>
                <Button size="sm" variant="outline" className="gap-1.5" data-testid="button-full-protocol">
...
function ProductReference() {
  ...
  return (
    <div className="space-y-3 max-w-2xl">
      ...
      <div className="space-y-2">
        {filtered.map((f) => (
          <Card key={f.brand} className={f.ha ? "" : "border-orange-200 dark:border-orange-900"} data-testid={`card-product-${f.brand.replace(/\s+/g, "-").toLowerCase()}`}>
            <CardContent className="p-3">
      ...
      <div className="max-w-4xl mx-auto px-4 py-6">
        <Tabs defaultValue="hyaluronidase">
          <TabsList className="mb-6 flex-wrap h-auto gap-1">
            <TabsTrigger value="hyaluronidase" className="text-xs gap-1.5" data-testid="tab-hyaluronidase">
              <Syringe className="h-3.5 w-3.5" /> Hyaluronidase
            </TabsTrigger>
            <TabsTrigger value="dilution" className="text-xs gap-1.5" data-testid="tab-dilution">
              <FlaskConical className="h-3.5 w-3.5" /> Toxin Dilution
            </TabsTrigger>
            <TabsTrigger value="dosing" className="text-xs gap-1.5" data-testid="tab-dosing">
              <Calculator className="h-3.5 w-3.5" /> Toxin Dosing
            </TabsTrigger>
            <TabsTrigger value="protocols" className="text-xs gap-1.5" data-testid="tab-protocols">
              <BookOpen className="h-3.5 w-3.5" /> Protocols
            </TabsTrigger>
            <TabsTrigger value="products" className="text-xs gap-1.5" data-testid="tab-products">
