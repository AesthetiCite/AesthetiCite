...
import {
  ...
  Syringe, Clock, AlertCircle, TrendingUp,
      ...
      <div className="max-w-3xl mx-auto px-4 py-6 space-y-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            {COMPLICATIONS.map((c) => (
              <button
                key={c.id}
                type="button"
                data-testid={`button-complication-${c.id.replace(/\s+/g, "-")}`}
            ...
            <div className="flex flex-wrap gap-1.5">
              {SYMPTOM_CHIPS.map((s) => (
                <button
                  key={s}
                  type="button"
                  data-testid={`chip-symptom-${s.toLowerCase().replace(/\s+/g, "-")}`}
        ...
        {result && !mutation.isPending && (
          <div ref={resultsRef} className="space-y-4">
            ...
            <WorkflowRunner
              complication={result.workflow_key ?? selected ?? customQuery}
              className="mt-0"
            />
            ...
            {result.safety && (
              <SafetyEscalationBlock
                ...
                sectionNumber={result.hyaluronidase ? "4" : "3"}
                className="mt-0"
              />
            )}
            ...
            <div className="flex flex-wrap gap-2 pt-2">
              ...
              <Button
                variant="outline"
                size="sm"
                data-testid="button-copy-decision"
                onClick={() => {
                  const text = [
              ...
              <Button variant="outline" size="sm" onClick={() => window.print()} data-testid="button-print-decision">
                <Printer className="h-3.5 w-3.5 mr-1.5" /> Print
              </Button>
              <LogCaseButton
                complication={result.workflow_key?.replace(/_/g, " ") ?? ""}
