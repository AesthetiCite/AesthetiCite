 ...
 *
 * Every complication must be documented. This makes it ONE TAP.
 *
 * Pre-filled from session context, 12-section MDO-standard form,
 * live preview, Export PDF / Copy / Share.
 */
...
async function downloadPdf(form: ReportForm & { evidence_refs?: any[] }): Promise<void> {
  const token = getToken();
...
function ReportPreview({ sections }: { sections: ReportSections }) {
  ...
  return (
    <div
      className="font-mono text-xs space-y-3 bg-slate-950 text-slate-200 rounded-xl p-4 max-h-[400px] overflow-y-auto"
...
// ── Main MedicoLegalReport component ─────────────────────────────────────────
...
export function MedicoLegalReport({
  ...
  const handleDownloadPdf = async () => {
    setPdfLoading(true);
    try {
      await downloadPdf({ ...form, evidence_refs: evidenceRefs });
  ...
  const handleCopy = () => {
    ...
    navigator.clipboard.writeText(sectionsToText(reportData.sections));
  ...
  const handleShare = async () => {
    ...
    if (navigator.share) {
      try {
        await navigator.share({ title: `AesthetiCite Incident Report — ${form.complication}`, text });
  ...
  const formContent = (
    <div className="space-y-1">
      <Accordion type="multiple" defaultValue={["complication", "treatment"]}>
      ...
      {step === "preview" && reportData && (
        <div className="space-y-4">
          <div className="grid grid-cols-3 gap-2">
            ...
            <Button variant="outline" onClick={handleCopy} className="gap-1.5" data-testid="button-copy-report">
              <Copy className="h-3.5 w-3.5" />
              Copy
            </Button>
            <Button variant="outline" onClick={handleShare} className="gap-1.5" data-testid="button-share-report">
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <p className="text-xs text-muted-foreground">
            All fields optional except complication type. Pre-filled from your session.
          </p>
        </DialogHeader>
        {content}
      </DialogContent>
...
// ── ReportExportButton — drop-in for any page ─────────────────────────────────
...
export function ReportExportButton({
  ...
  return (
    <MedicoLegalReport
      ...
      trigger={
        <Button size={size} className="gap-1.5" data-testid="button-open-report">
