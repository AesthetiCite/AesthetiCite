 ...
 * Shown below the evidence answer once streaming is complete.
 */
  ...
  async function exportPDF() {
    ...
    try {
      ...
      const res = await fetch("/api/complications/export-pdf", {
        ...
        body: JSON.stringify({ query: data._query, mode: "decision_support" }),
      });
          ...
          <div className="space-y-1 flex-1">
            <div className="flex flex-wrap items-center gap-2">
              <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold ${style.badge}`}>
                <AlertTriangle className="w-3 h-3" />PROTOCOL
              </span>
