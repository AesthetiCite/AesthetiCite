 ...
 *
 *  ImageAnnotator         — canvas drawing / markup layer
 *  MeasurementRuler       — click-to-measure distance overlay
 *  SerialDeltaDisplay     — quantified score diff between visits
 *  AnalysisConfidenceBadge— badge with tooltip
 *  ColourCalibrationGuide — pre-capture calibration instructions
 *  SharedReviewBanner     — expired/live shared review indicator
 *  PopulationBaseline     — expected appearance card
 *  AutoLogButton          — 1-click session logging
 */
...
// ═══════════════════════════════════════════════════════════════════════════
// 1. Image Annotator — canvas drawing layer
// ═══════════════════════════════════════════════════════════════════════════
...
export function ImageAnnotator({ imageUrl, onAnnotationsChange, onExport, height = 400 }: ImageAnnotatorProps) {
  ...
  const redraw = useCallback(() => {
    ...
    all.forEach(ann => {
      ...
      if (ann.tool === "pen" && ann.points.length >= 2) {
        ctx.beginPath();
    ...
    <div className="space-y-3">
      <div className="flex items-center gap-2 flex-wrap">
        <div className="flex items-center gap-1 bg-gray-100 dark:bg-gray-800 rounded-lg p-1">
          {TOOLS.map(t => (
            <button
              ...
              data-testid={`button-annotate-${t.key}`}
        ...
        <button
          ...
          data-testid="button-annotate-clear"
        >
          Clear
        </button>
        {onExport && (
          <button
            ...
            data-testid="button-annotate-export"
          >
            Save annotated
          </button>
        )}
  ...
  const handleClick = (e: React.MouseEvent) => {
    ...
    if (points.length >= 2) {
      ...
      if (next.length === 2 && mode === "calibrate") {
        ...
        setPxPerMm(dist / referenceDistanceMm);
        setCalibrated(true);
        setMode("measure");
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <div className="flex items-center gap-1 bg-gray-100 dark:bg-gray-800 rounded-lg p-1">
          <button
            ...
            data-testid="button-ruler-calibrate"
            className={`px-2.5 py-1 text-xs rounded-md font-medium transition-colors
          ...
          <button
            ...
            disabled={!calibrated}
            data-testid="button-ruler-measure"
    ...
    <div className="rounded-xl border border-border overflow-hidden">
      <div className={`px-4 py-3 border-b border-border/50 ${trajectoryColor}`}>
        <div className="flex items-center justify-between">
          <p className="text-sm font-bold capitalize" data-testid="text-serial-trajectory">
  ...
  return (
    <div className="space-y-1">
      <button
        ...
        data-testid="button-confidence-badge"
    ...
    <div className="rounded-xl border border-yellow-200 dark:border-yellow-800 bg-yellow-50 dark:bg-yellow-900/20 p-4 space-y-3">
      ...
      <div className="flex items-center gap-3">
        <div className="flex gap-2">
          {(["Good", "Avoid", "Avoid"] as const).map((label, i) => (
        ...
        {onDismiss && (
          <button
            ...
            data-testid="button-calibration-dismiss"
          >
            Got it
          </button>
        )}
...
export function SharedReviewBanner({ token: _token, expiresAt, shareUrl, onCopy }: ShareBannerProps) {
  ...
  return (
    <div className="rounded-xl border border-border overflow-hidden">
      <button
        ...
        onClick={() => setExpanded(v => !v)}
        data-testid="button-population-baseline-toggle"
      >
...
export function AutoLogButton({ sessionData, token, onLogged }: AutoLogButtonProps) {
  ...
  if (state === "done") {
    return (
      <div className="flex items-center gap-2 text-sm text-green-600 dark:text-green-400" data-testid="status-auto-log-done">
