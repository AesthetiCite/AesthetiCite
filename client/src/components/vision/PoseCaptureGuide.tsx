 ...
 * Usage:
 *   <PoseCaptureGuide
 *     onComplete={(files) => setSelectedFiles(files)}
 *     mode="serial"   // "serial" = 3 poses; "single" = front only
 *   />
 */

import { useState, useRef, useCallback } from "react";
import { Camera, CheckCircle2, ChevronRight, RefreshCw, User, AlertTriangle } from "lucide-react";
    ...
    <div className={`rounded-xl border-2 transition-all duration-200 overflow-hidden
      ...
      >
        <input
          ...
          data-testid={`input-pose-${pose.key}`}
        />
        ...
        {captured && isActive && (
          <button
            ...
            data-testid={`button-pose-retake-${pose.key}`}
          >
...
export function PoseCaptureGuide({
  ...
  const handleSubmit = () => {
    if (canSubmit) {
      setSubmitted(true);
      onComplete(captured as Record<string, File>);
    }
  };
  ...
  if (submitted) {
    return (
      <div className="flex items-center gap-3 rounded-xl border border-green-300 dark:border-green-700 bg-green-50 dark:bg-green-900/20 p-4">
        ...
        <button
          data-testid="button-pose-restart"
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-start justify-between">
        <div>
          <h3 className="text-sm font-semibold text-foreground flex items-center gap-2">
            ...
            {mode === "serial" && (
              <span className="text-xs font-normal text-muted-foreground ml-1">— 3-angle protocol</span>
            )}
          </h3>
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          data-testid="button-pose-analyse"
