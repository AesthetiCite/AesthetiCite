...
aesthetic complications. This is the feature that separates
AesthetiCite from every other evidence platform.

Design principles:
  1. ACTIONS, not text — every step is a verb you execute
  2. Timers — steps that require waiting have a countdown
  3. Branch logic — visual symptoms → immediate escalation path
  4. < 0ms retrieval — pure in-memory, zero database calls
  5. One endpoint: GET /api/workflow?complication=vascular_occlusion

Step types:
  "action"    — do something now (inject, call, apply)
  "assess"    — check / measure something
  "monitor"   — watch over time
  "escalate"  — if condition met → branch
  "document"  — record something
  "wait"      — timed pause (with timer_seconds)
"""
...
class WorkflowStep(BaseModel):
    ...
    type: StepType
    priority: Priority
    action: str                          # SHORT verb — the headline
    detail: str                          # explanation / rationale
    timer_seconds: Optional[int] = None  # countdown timer if applicable
    drug: Optional[str] = None          # drug name if this is a medication step
    dose: Optional[str] = None          # dose
    branch: Optional[Dict[str, Any]] = None  # conditional branch logic
    checkpoint: Optional[str] = None    # "What to look for before proceeding"
...
class Workflow(BaseModel):
    ...
    time_critical: Optional[str] = None   # e.g. "60-minute window"
    total_steps: int
    ...
    "vascular_occlusion": Workflow(
        ...
        steps=[
            ...
            WorkflowStep(
                ...
                timer_seconds=300,   # 5-minute repeat timer
            ...
            WorkflowStep(
                ...
                timer_seconds=3600,  # 60-minute escalation timer
                branch={
    ...
    "anaphylaxis": Workflow(
        ...
        steps=[
            ...
            WorkflowStep(
                ...
                timer_seconds=300,  # 5-min repeat timer
            ),
            WorkflowStep(
                step=3, type="action", priority="critical",
            ...
            WorkflowStep(
                ...
                timer_seconds=120,  # 2-minute reassessment
            ),
            WorkflowStep(
                step=6, type="action", priority="high",
    ...
    "dir": Workflow(
        ...
        steps=[
            ...
            WorkflowStep(
                ...
                detail="Photograph. Treatment given. Trigger identified. Review date. Counsel: may recur with future triggers. Consider not retreating if recurrent.",
            ),
        ],
    ),
...
# Update total_steps to match actual steps
...
def get_workflow(
    ...
    Returns the complete structured workflow for a complication.
    ...
    # Fuzzy match on key or display name
    for key, wf in WORKFLOWS.items():
