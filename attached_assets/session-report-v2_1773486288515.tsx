 ...
 *
 * Changes from v1:
 *   - Added "Generate Clinical Note" button after session is complete
 *   - Calls POST /api/growth/patient-readable-export
 *   - Displays plain-language summary suitable for patient record
 *   - Copy-to-clipboard on clinical note output
 */
...
function ClinicalNotePanel({ items, meta }: { items: QueueItem[]; meta: SessionMeta }) {
  ...
  // Build a plain-text source summary from the session results
...
export default function SessionReportPage() {
  ...
  async function runItem(id: string) {
    ...
    try {
      ...
      const res = await fetch("/api/safety/preprocedure-check", {
        ...
        body: JSON.stringify({ procedure: item.procedure, region: item.region, product_type: item.product_type,
          technique: item.technique, injector_experience_level: item.injector_experience_level, patient_factors: item.patient_factors }),
      });
      const data = await res.json();
  ...
  async function saveSession() {
    ...
    try {
      ...
      const createRes = await fetch("/api/growth/session-reports", { method: "POST", headers: h,
      ...
      for (const item of checked) {
        await fetch(`/api/growth/session-reports/${sid}/items`, { method: "POST", headers: h,
          body: JSON.stringify({ patient_label: item.patient_label || `Patient ${queue.indexOf(item) + 1}`,
