...
Strategy:
  Phase A — Journal-targeted ingestion (highest yield, lowest duplication)
             Pull by ISSN from 14 high-value journals M2/M4 missed
  ...
  Phase C — Multilingual expansion (French, Spanish, Portuguese, German)
...
Env:
    ...
    AI_INTEGRATIONS_OPENAI_API_KEY=...   # same key as FastAPI
...
# With API key: 10 req/s — safe floor is 0.12s between requests
# Without API key: 3 req/s — safe floor is 0.4s
REQUEST_DELAY = 0.12 if NCBI_API_KEY else 0.4

CHUNK_SIZE = 800        # characters per chunk — consistent with M2/M4
...
PHASE_A_JOURNALS = [
    ...
    ("BMJ Open",                               "2044-6055", 2018, 2026),  # patient safety papers
]
...
def fetch_abstract(pmid: str) -> str:
    ...
    try:
        ...
        r.raise_for_status()
        text = r.text.strip()
        # Strip the citation header line (first 2 lines from efetch text output)
        lines = text.split("\n")
...
def parse_summary(pmid: str, summary: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    return {
        ...
        "publication_types": pub_types_str,
        "specialty_tag": "",  # set downstream based on query context
    }


def make_source_id(pmid: str) -> str:
    return f"PMID_{pmid}"
...
def make_chunks(
    ...
    # Approx chunk_size chars — use word boundaries
    current: List[str] = []
    current_len = 0
    for word in words:
        current.append(word)
...
# ─────────────────────────────────────────────────────────────────────────────
# Embedding — fastembed, same model as production
# ─────────────────────────────────────────────────────────────────────────────
...
def pmid_exists(conn, pmid: str) -> bool:
    with conn.cursor() as cur:
        cur.execute("SELECT 1 FROM documents WHERE pmid = %s LIMIT 1;", (pmid,))
        return cur.fetchone() is not None


def insert_document(conn, doc: Dict[str, Any], specialty_tag: str = "") -> Optional[str]:
    """Insert document, return new doc id or None if already exists."""
    pmid = doc["pmid"]
    if pmid_exists(conn, pmid):
        return None
    doc_id = str(uuid.uuid4())
    with conn.cursor() as cur:
        cur.execute("""
            INSERT INTO documents
                (id, pmid, title, abstract, journal, year, authors, doi, url,
                 language, document_type, publication_types, specialty_tag, updated_at)
            VALUES
                (%s, %s, %s, %s, %s, %s, %s, %s, %s,
                 %s, %s, %s, %s, NOW())
            ...
            RETURNING id;
        """, (
            doc_id, pmid, doc["title"], doc["abstract"],
            doc["journal"], doc["year"], doc.get("authors", ""),
            doc.get("doi", ""), doc["url"],
            doc["language"], doc["document_type"],
...
def insert_chunks(conn, doc_id: str, chunks: List[str], embeddings: List[List[float]]) -> int:
    ...
    with conn.cursor() as cur:
        for i, (chunk, emb) in enumerate(zip(chunks, embeddings)):
            ...
            """, (chunk_id, doc_id, i, chunk, emb_str))
            inserted += 1
    return inserted
...
def ingest_pmids(
    ...
    new_pmids = [p for p in pmids if not pmid_exists(conn, p)]
    if not new_pmids:
    ...
    # Process in summary batches of 200 (NCBI limit)
    batch_size = 200
    chunk_embed_batch: List[Tuple[str, str, List[str]]] = []  # (doc_id, chunk_text)
    ...
    for batch_start in range(0, len(new_pmids), batch_size):
        ...
        for pmid in batch:
            ...
            # Fetch full abstract if esummary truncated it and we want full text
            if fetch_full_abstracts and len(doc.get("abstract", "")) < 200:
                full_abs = fetch_abstract(pmid)
                if full_abs:
                    doc["abstract"] = full_abs

            doc_id = insert_document(conn, doc, specialty_tag=specialty_tag)
            if not doc_id:
                continue  # was a conflict

            docs_inserted += 1

            # Build chunks for embedding
            chunks = make_chunks(doc_id, doc["title"], doc.get("abstract", ""))
            for chunk_text in chunks:
                chunk_embed_batch.append((doc_id, chunk_text))

            # Embed and flush in batches of BATCH_EMBED docs worth of chunks
...
def run_phase_a(dry_run: bool = False) -> None:
    ...
    for journal_name, issn, year_from, year_to in PHASE_A_JOURNALS:
        ...
        # Use journal name as specialty tag for this batch
        tag = "aesthetic_medicine" if any(
            kw in journal_name.lower()
            for kw in ["aesthetic", "cosmetic", "dermatologic", "plastic", "laser"]
        ) else "dermatology"

        docs, chunks = ingest_pmids(pmids, specialty_tag=tag, dry_run=dry_run)
        total_docs += docs
        total_chunks += chunks
    ...
    conn.close()
    logger.info(f"\nPhase A complete. Documents: {start_count:,} → {end_count:,} (+{end_count - start_count:,})")
...
def run_phase_b(dry_run: bool = False) -> None:
    ...
    for query, years in PHASE_B_QUERIES:
        ...
        # Derive specialty tag from query content
        tag = "aesthetic_medicine"
    ...
    end_count = get_doc_count(conn)
    conn.close()
    logger.info(f"\nPhase B complete. Documents: {start_count:,} → {end_count:,} (+{end_count - start_count:,})")
...
def run_phase_c(dry_run: bool = False) -> None:
    ...
    for query, lang, years in PHASE_C_QUERIES:
        ...
        docs, chunks = ingest_pmids(pmids, specialty_tag="aesthetic_medicine", dry_run=dry_run)
        total_docs += docs
        logger.info(f"  Inserted: {docs} docs, {chunks} chunks")
    ...
    end_count = get_doc_count(conn)
    conn.close()
    logger.info(f"\nPhase C complete. Documents: {start_count:,} → {end_count:,} (+{end_count - start_count:,})")
...
def print_status() -> None:
    ...
    logger.info(f"Post-2020 : {years['recent']:,} ({years['recent']*100//doc_count if doc_count else 0}%)")
