...
# The 20 most common complication queries in aesthetic medicine.
# These are precomputed at startup and served from memory.
# Add to this list as query analytics reveal new hot queries.
...
# Build alias lookup
_ALIAS_INDEX: Dict[str, str] = {}  # normalised alias → canonical query
...
def is_hot_complication(query: str) -> Tuple[bool, Optional[str]]:
    ...
    Matches exact normalised form or known aliases.
    """
    norm = _norm(query)
...
class HotAnswerCache:
    """
    Thread-safe in-memory cache for precomputed answers.
    TTL: 1 hour for hot answers (refreshed by background thread).
    """
...
def get_hot_answer(query: str) -> Optional[Dict]:
    """Check both the exact query and its canonical form."""
    # Direct lookup
...
def set_hot_answer(query: str, result: Dict) -> None:
    """Store answer under both the query and its canonical form."""
...
# Complication queries need fewer but higher-quality chunks.
# The LLM doesn't benefit from 12 chunks for "vascular occlusion protocol" —
# it needs the top 4–6 guideline/consensus sources.

CONTEXT_LIMITS = {
    "complication":          6,   # vascular, anaphylaxis, necrosis
    "high_risk_anatomy":     6,
    "technique":             8,
    "general":              10,
    "deepconsult":          14,   # no limit reduction for deep research
}

# Keywords that signal a complication query
...
def limit_context(
    ...
    Args:
      chunks        — ranked evidence chunks (should already be sorted by quality)
      query         — the user's query
      mode          — "auto" | "complication" | "technique" | "general" | "deepconsult"
      is_deepconsult— if True, no trimming (deep research needs full context)

    Returns trimmed chunk list.
    """
    if is_deepconsult:
        return chunks
    ...
    # Always keep guidelines and consensus statements, even if over limit
...
def estimated_prompt_tokens(chunks: List[Dict[str, Any]]) -> int:
    """Rough estimate: 1 token ≈ 4 chars. Useful for logging."""
...
def precompute_hot_queries(
    ...
    The embedding vectors for hot queries are cached in the retrieval pool.
    """
    import threading
    ...
    def _run():
        ...
        for hq in HOT_QUERIES[:max_queries]:
            ...
            try:
                t0 = time.time()
                # Just trigger the retrieval — the result caches in pgvector / retrieve_db's LRU
...
class LatencyTracker:
    """Rolling average latency per query mode. Used to log and monitor."""
