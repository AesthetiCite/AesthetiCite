...
Improvement 2 — Structured numeric output
  Extracts 6 trackable clinical scores from GPT-4o free text — no second LLM call.
...
Integration — add to main.py:
  ...
  app.include_router(vision_quality_router)

And add to server/routes.ts:
  app.post("/api/visual/validate-capture",  (req, res) => proxyToFastAPI(req, res, "/visual/validate-capture"));
  app.post("/api/visual/fitzpatrick",        (req, res) => proxyToFastAPI(req, res, "/visual/fitzpatrick"));
  app.post("/api/visual/training-case",      (req, res) => proxyToFastAPI(req, res, "/visual/training-case"));

In visual_counseling.py, replace your existing system/user prompt with:
  from app.engine.vision_quality import build_injectable_safety_prompt
  prompt = build_injectable_safety_prompt(question, context_hint=procedure_context)

After GPT-4o returns analysis_text, call:
  from app.engine.vision_quality import extract_visual_scores
  scores = extract_visual_scores(analysis_text)
  # add scores to your response payload: response["visual_scores"] = scores.dict()
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# IMPROVEMENT 3 — Hardened injectable safety prompt
# Replaces the generic GPT-4o vision prompt used in visual_counseling.py
# ─────────────────────────────────────────────────────────────────────────────
...
def build_injectable_safety_prompt(
    ...
    Usage in visual_counseling.py:
        ...
        # Replace your existing system prompt and user prompt with:
...
# ─────────────────────────────────────────────────────────────────────────────
# IMPROVEMENT 2 — Structured numeric output
# Extracts 6 trackable scores from the hardened prompt's analysis text.
# Zero extra LLM calls — pure regex/keyword extraction.
# ─────────────────────────────────────────────────────────────────────────────

class VisualScores(BaseModel):
    """
    Six numeric scores extracted from the hardened prompt output.
    All scores 0–3 unless noted. None = not assessable from image.
    These are clinician-trackable over serial visits.
    """
    ...
    asymmetry_flag: Optional[bool] = Field(
        None, description="True if visible asymmetry noted"
    )
    infection_signal: Optional[int] = Field(
        None, ge=0, le=3,
        description="0=none, 1=erythema only, 2=swelling+erythema, 3=fluctuance/purulent"
    )
    ptosis_flag: Optional[bool] = Field(
        None, description="True if eyelid/brow ptosis noted"
    )
    fitzpatrick_type: Optional[str] = Field(
        None, description="I, II, III, IV, V, or VI"
    )
    tyndall_flag: Optional[bool] = Field(
        None, description="True if blue-grey discolouration suggesting Tyndall effect noted"
    )
...
def extract_visual_scores(analysis_text: str) -> VisualScores:
    ...
    Usage:
        scores = extract_visual_scores(analysis_text)
        response_payload["visual_scores"] = scores.dict()
    """
...
# Scores the uploaded image before GPT-4o analysis.
# Tells the clinician if the photo is usable.
# ─────────────────────────────────────────────────────────────────────────────
...
async def validate_capture(file: UploadFile = File(...)) -> CaptureValidationResponse:
    """
    Accepts an uploaded image and returns a quality score + actionable feedback
    before committing to a full GPT-4o analysis call.

    Frontend: call this immediately on file select. If usable=false, show the
    suggestions and ask the clinician to retake before proceeding.
    """
    ...
    try:
        ...
        # Graceful fallback — don't block the clinician
...
# Dedicated endpoint for Fitzpatrick type + clinical implications.
# ─────────────────────────────────────────────────────────────────────────────
...
async def detect_fitzpatrick(file: UploadFile = File(...)) -> FitzpatrickResponse:
    """
    Estimates Fitzpatrick type I–VI from an uploaded photo.
    Returns clinical implications relevant to aesthetic practice:
    PIH risk, laser/energy device risk, and keloid risk.

    Important: this is an estimate only. Photo lighting and filters
    significantly affect apparent skin tone.
    """
    img_bytes = await file.read()
    b64 = base64.b64encode(img_bytes).decode()
...
# ─────────────────────────────────────────────────────────────────────────────
# IMPROVEMENT 5 — Fine-tune dataset accumulation
# POST /visual/training-case
# Structured case schema for building the proprietary injectable visual dataset.
# When enough cases are accumulated, this is the training data for a fine-tuned
# model that outperforms raw GPT-4o on injectable complication detection.
# ─────────────────────────────────────────────────────────────────────────────

class TrainingCaseRequest(BaseModel):
    """
    A single labelled visual case for dataset accumulation.
    Clinician confirms or corrects what the AI detected.
    These labels are ground truth for future fine-tuning.
    """
    # Image reference
    ...
    clinician_diagnosis: Optional[str] = Field(
        None,
        description="e.g. 'vascular occlusion', 'Tyndall effect', 'normal post-procedure swelling'",
    )
    clinician_corrected_protocol: Optional[str] = Field(
        None,
        description="If AI triggered wrong protocol, what should it have been",
    )
    outcome: Optional[Literal["resolved", "improving", "stable", "worsening", "referred", "unknown"]] = None

    # AI was right or wrong
    ai_assessment_correct: Optional[bool] = Field(
        None,
        description="True if AI signals matched clinician ground truth",
    )
...
# In-memory accumulator — swap for Postgres when you have >100 cases
_TRAINING_CASES: List[Dict[str, Any]] = []

# Dataset statistics for model readiness tracking
...
def log_training_case(req: TrainingCaseRequest) -> TrainingCaseResponse:
    """
    Accepts a clinician-labelled visual case.
    Builds the structured dataset needed to eventually fine-tune a model
    specifically on injectable complication visual patterns.

    Consent gate: consent_obtained must be True before this is accepted.
    Target: 500+ labelled cases per complication type for a viable fine-tune run.
    """
...
def training_dataset_stats() -> Dict[str, Any]:
    """
    Returns dataset size, label distribution, AI accuracy rate,
    and an estimate of readiness for fine-tuning.

    Fine-tune readiness thresholds (rough guidance):
      - 100+ cases:  enough for initial prompt/few-shot optimisation
      - 500+ per class: viable for LoRA fine-tune on GPT-4o or open model
      - 2000+ labelled: approaching specialist-model territory
    """
