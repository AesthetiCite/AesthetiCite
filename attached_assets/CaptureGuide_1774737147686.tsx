 ...
 *
 * Usage: replace your current file-input with this component in VisualCounselingPage.
 *
 ...
 *     // proceed with upload + analysis
 *   }}
 * />
 *
 * Flow:
 *   1. Show clinician capture instructions (checklist)
 *   2. On file select → call POST /api/visual/validate-capture
 *   3. Show quality score + issues
 *   4. If usable → emit onImageReady
 *   5. If not usable → show issues + suggestions, allow retry
 */
...
interface CaptureGuideProps {
  ...
  token?: string;
  /** Skip validation and just emit file immediately — for testing */
...
export function CaptureGuide({ onImageReady, token, skipValidation = false }: CaptureGuideProps) {
  ...
  const handleFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      if (data.usable) {
        // Auto-advance after 1.5s so clinician sees the pass
        setTimeout(() => {
          setPhase("ready");
      ...
      // If validation fails entirely, don't block — pass through
      ...
      <div className="rounded-xl border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900/50 p-5 space-y-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            Consistent photos make serial comparisons reliable. Check these before taking the image.
          </p>
        </div>
  ...
  // ── Phase: ready (analysis has been handed off) ───────────────────────────
  if (phase === "ready") {
    return (
