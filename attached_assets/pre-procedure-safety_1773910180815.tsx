...
interface PatientFactors {
  ...
  smoking?: boolean;
  glp1_patient?: boolean;       // NEW: GLP-1 / semaglutide / tirzepatide
}
...
interface ProcedureInsight {
  ...
  ultrasound_recommended?: boolean;     // NEW
  ultrasound_note?: string;             // NEW
}
...
function computeDifferentials(symptoms: DiffSymptoms): DifferentialResult[] {
  ...
  // Add fallback if no high-scoring differentials
  if (results.length === 0) {
    results.push({
      rank: 1,
...
export default function PreProcedureSafetyPage() {
  const [activeTab, setActiveTab] = useState<"check" | "differential">("check");

  // ── Safety Check form state ───────────────────────────────────────────────
            ...
            <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
              ...
              <div className="space-y-3">
                <SelectField
                  ...
                  options={[
                    { value: "immediate (during procedure)", label: "Immediate (during procedure)" },
                    { value: "minutes after", label: "Minutes after procedure" },
                    { value: "hours after", label: "Hours after procedure" },
