...
async def get_case_or_404(
    ...
    # Add ownership check here when auth is wired:
...
async def upsert_patient_context(
    ...
    patient_ctx = PatientContext(**payload.model_dump())
    db.add(patient_ctx)
    await db.commit()
...
async def upsert_procedure_context(
    ...
    proc_ctx = ProcedureContext(**payload.model_dump())
    db.add(proc_ctx)
    await db.commit()
...
async def start_protocol_run(
    ...
    # Initialise all steps from protocol definition
...
async def update_protocol_step(
    ...
    # Auto-advance: mark next step as ready when this one completes
    if payload.status == "completed":
        ...
        # Update run's current_step_order
...
async def create_disposition(
    ...
    # Auto-close case when disposition is set
    case.status = "closed"
