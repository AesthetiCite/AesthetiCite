 ...
 *
 * Route: /safety-workspace  (add to App.tsx)
 *
 * Add to App.tsx:
 ...
 *   </Route>
 *
 * Add to More dropdown and mobile Sheet:
 *   <Link href="/safety-workspace">
 *     <Activity className="h-4 w-4" /> Safety Workspace
 *   </Link>
 *
 * Wiring summary:
...
interface ProtocolCard {
  id: string;               // matched_protocol_key
  title: string;            // matched_protocol_name
  category: string;
  severity: Severity;
  immediateAction: string[];
  treatment: string[];      // escalation + monitoring + dose guidance
  followup: string[];       // follow_up_questions
  redFlags: string[];
  patientSummary: string;   // clinical_summary
  evidenceTier: EvidenceTier;
  confidence: number;       // 0-100
  sources: ProtocolSource[];
  recommendedDose?: string;
  requestId?: string;       // for PDF export
}
  ...
  const fetchLiveProtocol = useCallback(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      ...
      try {
        ...
        setLiveProtocol(null); // fall back to static
      } finally {
