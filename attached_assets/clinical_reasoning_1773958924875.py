...
REASONING_MODEL       = "gpt-4o"          # Best model — clinical reasoning needs it
REASONING_TEMPERATURE = 0.15              # Low — we want consistent, accurate output
...
def _build_prompt(
    ...
    # Build a structured case description
    case_parts = []
...
# ---------------------------------------------------------------------------
# Evidence context builder (pulls top chunks from existing RAG if available)
# ---------------------------------------------------------------------------
...
def _build_evidence_context(
    ...
    """
    Try to pull the top-ranked evidence chunks for the complication
    from the existing pgvector store to ground the LLM reasoning.
    Falls back to empty string if retrieval unavailable.
    """
    try:
...
async def _stream_reasoning_llm(prompt: str) -> AsyncGenerator[str, None]:
    ...
    Uses the same httpx pattern as ask_v2.py.
    """
    import httpx
...
async def _call_reasoning_llm_sync(prompt: str) -> Dict[str, Any]:
    """Non-streaming call — assembles full JSON, parses, returns."""
    full = ""
...
def _fallback_reasoning(prompt: str) -> Dict[str, Any]:
    """
    Return a minimal valid structure if LLM fails.
    Extracts complication type from prompt for basic output.
    """
...
async def stream_reasoning(
    ...
    Emits events:
      {type: "start"}
      {type: "token", content: "..."}     ← raw LLM tokens as they arrive
      {type: "reasoning", data: {...}}    ← parsed JSON once complete
      {type: "done"}
      {type: "error", message: "..."}
    """
    ...
    async def generate() -> AsyncGenerator[str, None]:
        ...
        # Stream tokens (so the frontend can show a typing indicator)
        full_text = ""
        try:
            async for tok in _stream_reasoning_llm(prompt):
                full_text += tok
                # Emit tokens so UI knows it's working
        ...
        # Emit the parsed structured result
...
async def check_cache(
    ...
    """Check if reasoning is cached for a complication — useful for pre-warming."""
