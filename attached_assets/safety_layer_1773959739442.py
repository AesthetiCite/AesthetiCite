...
Output structure (matches build plan exactly):
  {
    ...
    "when_to_stop":  [str],
    "when_to_refer": [str],
    "escalation":    str,           ← single most important escalation criterion
    "escalation_criteria": [str],   ← full list
    "monitoring":    [str],
...
# ===========================================================================
# QUERY CLASSIFICATION  (determines which safety ruleset fires)
# ===========================================================================

# Complication triggers — any of these → full complication safety block
...
# High-risk anatomy — any of these → elevated safety rules
...
# Technique queries — dosing/technique → IFU safety block
...
def classify_query(query: str) -> Tuple[str, int]:
    ...
    query_type: "complication" | "high_risk_anatomy" | "technique" | "regulatory" | "general"
    risk_base_score: 0-100
    """
    q = query.lower()

    if any(t in q for t in _COMPLICATION_TRIGGERS):
        # Check for life-threatening subtypes
...
def assess_safety(
    ...
    *,
    ...
    Parameters:
      ...
      symptoms                    — list of reported symptoms (optional)
      region                      — anatomical region (optional)
      procedure                   — procedure type (optional)
      existing_protocol_risk_level — if complication protocol already fired,
                                    pass its risk level to ensure consistency
    """
    ...
    # Upgrade risk level if complication protocol already identified something worse
    ...
    # Enrich with region-specific escalation if high-risk anatomy detected
...
def safety_block_for_prompt(query: str) -> str:
    ...
    Used in build_streaming_prompt() / _build_single_call_prompt() so the
    LLM's generated answer is already safety-aware.
    """
