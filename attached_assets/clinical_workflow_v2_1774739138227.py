...
Add to main.py:
    from app.api.clinical_workflow_v2 import router as clinical_workflow_router
...
_STORE: Dict[str, Dict[str, Any]] = {}   # swap for Postgres in production
...
async def generate_consultation_note(req: ConsultationNoteRequest) -> ConsultationNoteResponse:
    ...
    """
    # Merge pre-procedure data into the prompt context
    extra_context = ""
    ...
    # Merge pre-procedure result if provided
    if req.pre_procedure_result:
        sa = req.pre_procedure_result.get("safety_assessment", {})
...
class ConsentRequest(BaseModel):
    ...
    decision: Optional[str] = None          # go | caution | high_risk
    top_risks: List[str] = []
    ...
    # Signature capture (base64 image or electronic timestamp)
...
class ConsentResponse(BaseModel):
    ...
    audit_hash: str                          # SHA-256 of consent content
    generated_at_utc: str
    consent_text: str


def _build_consent_text(req: ConsentRequest) -> str:
    """Generate the consent document text from procedure data."""
...
def create_consent(req: ConsentRequest) -> ConsentResponse:
    """
    Generates a consent form pre-populated from the pre-procedure safety check.
    Stores with SHA-256 audit hash for medico-legal traceability.
    If patient_agreed and clinician_agreed are both True, status = 'signed'.
    """
...
# Red flag rules: each maps a condition → complication risk boost + flag message
RED_FLAG_RULES = [
    {
...
class PreflightQuestionnaire(BaseModel):
    """Patient self-reported data collected before appointment."""
...
class PreflightResult(BaseModel):
    ...
    has_critical: bool
    has_high: bool
    risk_score_modifier: int           # add to pre-procedure base score
...
def evaluate_preflight(req: PreflightQuestionnaire) -> PreflightResult:
    ...
    The clinician sees a "⚠ 2 red flags detected" alert when they open
    the patient's record — before the consultation begins.
    """
    ...
    result = PreflightResult(
        ...
        has_high=has_high,
        risk_score_modifier=min(score_modifier, 40),  # cap modifier at 40 pts
...
# Aesthetic medicine NER patterns (replaces a full ML NER model for now;
# wire in OpenMed HuggingFace models when available at production scale)
...
def tag_chunk_ner(text: str, title: str = "") -> Dict[str, Any]:
    ...
    # Derive priority signals for retrieval boosting
    is_safety_critical = bool(tags.get("complication") or tags.get("anatomical"))
    has_product        = bool(tags.get("product"))
    evidence_strength  = "strong" if "systematic review" in tags.get("evidence_type", []) \
                         or "meta-analysis" in tags.get("evidence_type", []) \
                         or "RCT" in tags.get("evidence_type", []) \
