 ...
 *   <VisionCompareResult>   — renders serial comparison results
 */
...
function PrimaryDiagnosisCard({
  ...
  return (
    <Card className={`border-2 ${urg.border}`}>
      <CardContent className="p-4 space-y-3">
        ...
        {diagnosis.complication && diagnosis.complication !== "normal_post_procedure" && (
          <Link href={`/decide?complication=${encodeURIComponent(diagnosis.display_name)}`}>
            <Button size="sm" className="w-full gap-1.5" variant={
              diagnosis.urgency === "immediate" ? "destructive" : "default"
            }>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between text-xs text-muted-foreground">
        ...
        {onReset && (
          <button type="button" onClick={onReset} className="hover:text-foreground flex items-center gap-1">
  ...
  return (
    <div className="space-y-4">
      ...
      {concern && concern.complication !== "normal_post_procedure" && (
        <div>
          <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-2">
            Primary Concern {concern.is_new_since_baseline && (
              <Badge className="text-[10px] ml-1 bg-red-100 text-red-700 border-red-300">NEW</Badge>
            )}
          </p>
...
export function VisionDiagnoseButton({
  ...
  const mutation = useMutation({
    ...
    onSuccess: (data) => {
      setResult(data);
      onResult?.(data);
      // Auto-alert for urgent/immediate
