/**
 * AesthetiCite — Complication Quick-Start
 * Replaces the generic "Ask anything" empty state.
 *
 * Design principle: clinicians in a live complication scenario
 * need zero friction. One tap → query fires.
 *
 * Renders:
 *   - Headline: "What complication are you facing?"
 *   - 8 large quick-select buttons (complication shortcuts)
 *   - Free-text input below (for anything not in the grid)
 *   - Recent searches (if any)
 *   - Subtle evidence quality badge
 *
 * Usage in ask.tsx:
 *   Replace the existing empty-state div with:
 *   <ComplicationQuickStart onQuery={handleAsk} isLoading={isLoading} />
 */
...
import {
  ...
  Eye, Microscope, Pill, HelpCircle,
} from "lucide-react";

// ---------------------------------------------------------------------------
// Complication quick-select options
// ---------------------------------------------------------------------------

interface QuickOption {
  id: string;
  label: string;
  query: string;           // the exact query fired
  icon: React.ElementType;
  urgent: boolean;         // red styling for life-threatening options
...
const QUICK_OPTIONS: QuickOption[] = [
  ...
  },
  {
    id: "other",
    label: "Other",
    query: "",               // no auto-query — focus the text input
    icon: HelpCircle,
    urgent: false,
  },
];
...
// ---------------------------------------------------------------------------
// Suggested follow-up queries shown below the grid
// ---------------------------------------------------------------------------
...
export function ComplicationQuickStart({
  ...
  // Keyboard shortcuts for urgent options
  useEffect(() => {
  ...
  const handleSelect = (opt: QuickOption) => {
    if (opt.id === "other" || !opt.query) {
      // Focus the text input for custom query
      setInput("");
  ...
  return (
    <div className={`flex flex-col items-center w-full max-w-2xl mx-auto ${className}`}>
      ...
      <div className="mt-6 flex items-center gap-2 text-xs text-muted-foreground">
        ...
        <span>206,000+ papers · Evidence ranked by clinical hierarchy</span>
      </div>
    </div>
  );
}
