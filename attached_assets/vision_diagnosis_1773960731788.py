...
Output structure (VisualDX-style ranked differential):
  {
    ...
    "trigger_protocol": str | null,   ← complication key for /api/decide
...
# The core VisualDX asset — visual signs mapped to complications
# ===========================================================================
...
async def _call_vision_llm(
    ...
    try:
        async with httpx.AsyncClient(timeout=60.0) as client:
            resp = await client.post(f"{_AI_BASE.rstrip('/')}/chat/completions",
...
async def diagnose_single(
    ...
    Form fields:
      ...
      procedure    — e.g. "lip filler"
      region       — e.g. "lips"
      time_since   — e.g. "20 minutes post-injection"
      product      — e.g. "Juvederm Ultra 1ml"
    """
...
async def diagnose_compare(
    ...
    POST /api/vision/diagnose-compare
    Form fields:
      baseline            — baseline (day 0) image
      followup            — follow-up image
      procedure           — procedure type
      region              — anatomical region
      days_since_baseline — e.g. "2 days"
    """
    t0 = time.perf_counter()
