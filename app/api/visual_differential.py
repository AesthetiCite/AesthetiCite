...
# Visual store — backed by disk + memory cache via visual_store.py
# _VISUAL_STORE kept as a thin shim so existing imports don't break.
from app.api import visual_store as _vs

class _VisualStoreShim(dict):
    """Dict-like shim — reads fall through to visual_store.load_image()."""
    def __contains__(self, key):
        return _vs.visual_exists(str(key))
    def __getitem__(self, key):
        data = _vs.load_image(str(key))
        if data is None:
            raise KeyError(key)
        return data
    def get(self, key, default=None):
        return _vs.load_image(str(key)) or default
    def pop(self, key, *args):
        _vs.delete_visual(str(key))
        return args[0] if args else None

_VISUAL_STORE: Dict[str, bytes] = _VisualStoreShim()

def register_visual(visual_id: str, image_bytes: bytes, content_type: str = "image/jpeg") -> None:
    """Called by the upload endpoint — persists image to disk + memory cache."""
    _vs.register_visual(visual_id, image_bytes, content_type)

def _load_image_bytes(visual_id: str) -> Optional[bytes]:
    """Load raw image bytes for a visual_id — memory → disk."""
    return _vs.load_image(visual_id)
